...
double **pressurePoissonPeriodic(double **p,double ** b, double dx, double dy,
                         int nx,int ny);
...
void run2DChannelFlow(int nx, int ny,int nt)
{
    ...
    for (int n = 0; n < nt; n++)
    {
        ...
        p = pressurePoissonPeriodic(p,b,dx,dy,nx,ny);
        ...
        for (int i=1; i<ny - 1; i++)
        {
            ...
            u[i][nx-1] = (un[i][nx-1] - (un[i][nx-1] * (dt/dx) *(un[i][nx-1] - un[i][nx-2]))
                            ...
                            + nu * ( (dt / pow(dx,2) ) * (un[i][0] - (2 * un[i][nx-1]) + un[i][nx-2] )
                            + ((dt/pow(dy,2)) * (un[i+1][nx-1] - (2*un[i][nx-1]) + un[i-1][nx-1]) ) )
                            + (F * dt));
            ...
            u[i][0] = (un[i][0] - (un[i][0] * (dt/dx) *(un[i][0] - un[i][nx-2]))
                            ...
                            + nu * ( (dt / pow(dx,2) ) * (un[i][1] - (2 * un[i][0]) + un[i][nx-1] )
                            + ((dt/pow(dy,2)) * (un[i+1][0] - (2*un[i][0]) + un[i-1][0]) ) )
                            + (F * dt));
            ...
            v[i][nx-1] = (vn[i][nx-1] - (un[i][nx-1] * (dt/dx) *(vn[i][nx-1] - vn[i][nx-2]))
                            ...
                            + nu * ( (dt / pow(dx,2) ) * (vn[i][0] - (2 * vn[i][nx-1]) + vn[i][nx-2] )
                            + ((dt/pow(dy,2)) * (vn[i+1][nx-1] - (2*vn[i][nx-1]) + vn[i-1][nx-1]) ) ) );

            // Periodic BC v @ x = 0
            v[i][0] = (vn[i][0] - (un[i][0] * (dt/dx) *(vn[i][0] - vn[i][nx-2]))
                            - (vn[i][0] * (dt/dy) *(vn[i][0] - vn[i-1][0]))
                            - ((dt/ (2*rho*dy)) * (p[i+1][0] - p[i-1][0]) )
                            + nu * ( (dt / pow(dx,2) ) * (vn[i][1] - (2 * vn[i][0]) + vn[i][nx-1] )
                            + ((dt/pow(dy,2)) * (vn[i+1][0] - (2*vn[i][0]) + vn[i-1][0]) ) ));
                         ...
                         int nx,int ny)
{
    ...
    for (int it = 0; it < nit; it++)
    {
        ...
        for (int i=1; i<ny - 1; i++)
        {
            ...
            p[i][nx-1] = ( ( ( (pn[i][0] + pn[i][nx-2]) * pow(dy,2) + (pn[i+1][nx-1] + pn[i-1][nx-1]) * pow(dx,2))
            / (2 * (pow(dx,2) + pow(dy,2))) )
            - ((pow(dx,2) * pow(dy,2)) / (2 * (pow(dx,2) + pow(dy,2)) ))
            * b[i][nx-1] );

            //Periodic BC for Pressure @ x = 0
            p[i][0] = ( ( ( (pn[i][1] + pn[i][nx-1]) * pow(dy,2) + (pn[i+1][0] + pn[i-1][0]) * pow(dx,2))
            / (2 * (pow(dx,2) + pow(dy,2))) )
            - ((pow(dx,2) * pow(dy,2)) / (2 * (pow(dx,2) + pow(dy,2)) ))
            * b[i][0] );
        }
